...
class OrdersViewModelTest {
    ...
    fun `init populates displayOrders from cache applying the default filter`() {
        // Default filter: OPEN + last 30 days. Both orders are open and recent.
    ...
    @Test
    fun `ALL shows every order regardless of status`() {
        val vm = vm(cache = listOf(order(1L, Status.WORKING), order(2L, Status.FILLED), order(3L, Status.CANCELED)))
        vm.filter.value = vm.filter.value.copy(statusFilter = OrderFilter.StatusFilter.ALL, lookbackDays = null)
    ...
    fun `OPEN hides terminal-status orders`() {
        ...
        vm.filter.value = vm.filter.value.copy(statusFilter = OrderFilter.StatusFilter.OPEN, lookbackDays = null)
        assertEquals(2, vm.displayOrders.size)
    }
    ...
    fun `FILLED shows only FILLED orders`() {
        ...
        vm.filter.value = vm.filter.value.copy(statusFilter = OrderFilter.StatusFilter.FILLED, lookbackDays = null)
        assertEquals(2, vm.displayOrders.size)
    }
    ...
    fun `CANCELED shows only CANCELED orders`() {
        ...
        vm.filter.value = vm.filter.value.copy(statusFilter = OrderFilter.StatusFilter.CANCELED, lookbackDays = null)
    ...
    fun `EXPIRED shows only EXPIRED orders`() {
        ...
        vm.filter.value = vm.filter.value.copy(statusFilter = OrderFilter.StatusFilter.EXPIRED, lookbackDays = null)
        assertEquals(1, vm.displayOrders.size)
    }
    ...
    fun `REJECTED shows only REJECTED orders`() {
        ...
        vm.filter.value = vm.filter.value.copy(statusFilter = OrderFilter.StatusFilter.REJECTED, lookbackDays = null)
        assertEquals(1, vm.displayOrders.size)
    }
    ...
    fun `lookback window excludes orders older than the cutoff`() {
        ...
        vm.filter.value = vm.filter.value.copy(
            statusFilter = OrderFilter.StatusFilter.ALL,
            lookbackDays = 30,
    ...
    fun `changing filter updates displayOrders synchronously`() {
        ...
        // Default is OPEN — only WORKING visible
        assertEquals(1, vm.displayOrders.size)
